...
当第一个参数为负数的时候，`slice`、`substr`表现相同，截取总长度加上负值后的结果到最后。而`substring`方法会将负数直接转换成0

当第二个参数为负数的时候，`slice`会将第二个参数加上length，截取第一个参数到这个数的这一段，`substring`还是会将第二位转换为0，`substr`也会将第二个参数改变为0。

位置方法：`indexOF`、`lastIndexOf`，跟数组相同

`trim`方法，会返回字符串的一个去掉前置和后置空格的副本

大写小写转换方法：`toLowerCase`、`toUpperCase`、`toLocaleLowerCase`、`toLocaleUpperCase`，在使用的时候最好使用locale，根据自己的时区来选择

字符串的模式匹配，`match`方法，它接受一个正则表达式或者regexp对象，返回一个数组，跟regexp对象的exec方法相同，返回一个数组。

`search`方法，跟`match`方法接收一样的参数，返回第一次匹配到的位置，如果没有匹配到，就返回-1

`replace`方法，接收两个参数，第一个参数跟以上两个方法相同，第二个参数是要替换的字符串，它会把匹配到的字符替换成我们传入的字符

`split`方法，接收两个参数，第一个参数为字符或者正则，第二个参数为数组最大length，它会根据传入的第一个参数将字符串分割为数组中的项，并且只返
回第二个参数指定的这么多项

`localCompare`:接受一个字符串，如果两个字符串相同，返回0，如果传入参数在原字符串之前，返回1，如果在之后返回-1